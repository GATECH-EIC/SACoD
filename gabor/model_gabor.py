...
class GaborConv():
	...
	def __init__(self):
		...
		self.filter = nn.Conv2d(1, 4, 11, padding=5, bias=True)
		gabor = loadmat('gabor.mat')['filts']
		gabor = np.rollaxis(np.rollaxis(gabor, 2), 3, 1)
		gabor = gabor.reshape(8, 1, 11, 11)
...
class NetworkCIFAR(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, auxiliary, genotype):
    ...
    C_curr = 4
    self.optical = GaborConv()
    self.bn1 = nn.BatchNorm2d(C_curr)
    C_curr = 8
    self.conv2 = nn.Conv2d(4, C_curr, 3, padding=1, bias=False)
    self.bn2 = nn.BatchNorm2d(C_curr)
  ...
  def forward(self, input):
    ...
    tmp = self.optical.forward(input)
    tmp = self.bn1(tmp)
    # s0 = s1 = self.bn1(tmp)
    tmp = self.conv2(tmp)
    s0 = s1 = self.bn2(tmp)
