...
def main():
    ...
    tmp = torch.randn(1, 3, 32, 32).cuda()
    model = model.cuda()
    flops, params = profile(model, inputs=(tmp,), custom_ops=custom_ops)
    logging.info("params = %fMB, FLOPs = %fGB", params / 1e6, flops / 1e9)

    flops_measured = model.forward_flops((3,32,32))
    logging.info("FLOPs measured = %fGB", flops_measured / 1e9)
    ...
    if type(config.pretrain) == str:
        ...
        # print(state_dict["module.optical_cnn.filter0.weight"])
